...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose([
        ...
        create_transform(**data_config, is_training=True)
    ])

    test_transform = transforms.Compose([
        transforms.Lambda(lambda img: img.convert('RGB') if img.mode != 'RGB' else img),  # Convert grayscale to RGB
        create_transform(**data_config, is_training=False)
    ])
    ...
    out_dict = train(model=model, optimizer=optimizer, criterion=loss_fn, num_epochs=10)
